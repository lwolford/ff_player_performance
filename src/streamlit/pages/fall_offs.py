...
st.title("📉 Running on Empty")
st.caption(read_page_content(__file__, "fall_offs.md"))
st.markdown("---")
...
def load_data():
    ...
    merged["decline"] = merged["first_avg"] - merged["second_avg"]
    merged = merged.sort_values("decline", ascending=False).reset_index(drop=True)

    return merged, build_preload_urls(merged)


fall_data, preload_urls = load_data()
preload_headshots(preload_urls)

selected = st.segmented_control("Position", POSITIONS, default="All", label_visibility="collapsed")
filtered = filter_by_position(fall_data, selected)
