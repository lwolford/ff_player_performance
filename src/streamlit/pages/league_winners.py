...
st.title("🏆 Set & Forget")
st.caption(read_page_content(__file__, "league_winners.md"))
st.markdown("---")
...
def load_data():
    ...
    df = df[["player_name", "headshot_url", "position", "fantasy_points_ppr"]]
...
penalty = st.session_state.get("league_penalty", 0.5)
scored = base_data.assign(
    league_winner_score=base_data["points_ppr_avg"] - penalty * base_data["points_ppr_std"]
)

filtered = filter_by_position(scored, selected)
top4 = filtered.nlargest(4, "league_winner_score")
...
for rank, (col, row) in enumerate(zip(cols, top4.to_dict("records")), start=1):
    with col:
        player_card(
            ...
            row["position"],
            row["headshot_url_lg"],
            round(row["league_winner_score"], 2),
            title="Weeks 1–17",
            rank=rank,
            label="Season Score",
        )
